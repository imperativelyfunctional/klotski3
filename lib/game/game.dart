...
class Klotski extends FlameGame with PanDetector {
  ...
  Future<void> onLoad() async {
    ...
    playingBGM = await FlameAudio.audioCache.loop('rain.mp3', volume: 0.1);
    _buildGameBoard();
  }
  ...
  void _buildGameBoard() async {
    ...
    if (gameArguments.newGame) {
      ...
      Mission mission = await _readMissionFromJson(id);
      ...
      if (game == null) {
        Mission mission = await _readMissionFromJson(1);
        positions = mission.piecePositions;
        name = mission.name;
        id = mission.id;
      } else {
        final jsonResult = json.decode(game);
        var mission = Mission.fromJson(jsonResult);
        name = mission.name;
        id = mission.id;
        positions = mission.piecePositions;
      }
    ...
    var pieces = await controller.layoutPieces(positions, this);
    var caoCao =
        pieces.firstWhere((element) => element.info == PieceInfo.caoCao);
    caoCao.callback = () {
      children.any((element) {
        if (element is Piece &&
